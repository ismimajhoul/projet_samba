...
static int atto320_power_get(struct tegracam_device *tc_dev)
{
	...
	dev_err(dev, "je suis la je fais un truc ici\n");
...
static int atto320_board_setup(struct atto320 *priv)
{
	...
	dev_err(dev, "ser_node reg %d\n",priv->g_ctx.ser_reg);

	ser_i2c = of_find_i2c_device_by_node(ser_node);
	of_node_put(ser_node);
	dev_err(dev, "seri2c_ name %s\n",ser_i2c->name);
	...
	{
		dev_err(dev, "deserializer driver name %s\n",dser_i2c->dev.driver->name);
	}
