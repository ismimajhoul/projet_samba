...
static int max9296_write_link(struct device *dev, u32 link)
{
	...
	if (link == GMSL_SERDES_CSI_LINK_A)
	{
		...
		dev_err(dev, "%s: gmsl link A selected\n", __func__);
	}
...
int max9296_setup_control(struct device *dev, struct device *s_dev)
{
	...
	//////
	/////
	max9296_read_reg(dev,0xBCA, &value_cr13);
	dev_err(dev, "after init: video lock result 0xBCA=0x%x \n",value_cr13);

	//lecture du gmsl1 link locked
	max9296_read_reg(dev,0xBCB, &value_cr13);
	dev_err(dev,"after init MAX9296 link locked value 0xBCB=0x%x\n",value_cr13 );
	////
	msleep(1000);
